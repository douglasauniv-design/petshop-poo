...
import MainController from "../src/control/MainController";
import { PorteAnimal } from "../src/enum/PorteAnimal";
import { TipoPelagem } from "../src/enum/TipoPelagem";
import { TipoAgua } from "../src/enum/TipoAgua";
import { TipoServico } from "../src/enum/TipoServico";
import ValidacaoException from "../src/exception/ValidacaoException";
...
describe("MainController", () => {
    ...
    beforeEach(() => {
        ...
        controller = new MainController(new Database());
    });

    test("cadastra um cachorro com sucesso", () => {
        controller.cadastrarCachorro("Rex", 3, "Labrador", 30, TipoPelagem.CURTA, PorteAnimal.GRANDE, true);
    ...
    test("cachorro também possui pelagem (herdada de Mamifero)", () => {
        const cachorro = controller.cadastrarCachorro("Rex", 3, "Labrador", 30, TipoPelagem.LONGA, PorteAnimal.GRANDE, true);
        expect(cachorro.getPelagem()).toBe(TipoPelagem.LONGA);
    });

    test("cadastra coelho e peixe (novos tipos)", () => {
        controller.cadastrarCoelho("Pipoca", 1, "Anão", 2, TipoPelagem.CURTA, true);
        controller.cadastrarPeixe("Nemo", 1, "Palhaço", 0.1, TipoAgua.SALGADA);
        expect(controller.quantidadeAnimais()).toBe(2);
    });

    test("lança exceção personalizada para idade inválida", () => {
        expect(() => {
            controller.cadastrarGato("Mimi", -1, "Siamês", 4, TipoPelagem.CURTA, false);
        }).toThrow(ValidacaoException);
    });

    test("lança exceção para nome vazio", () => {
        expect(() => {
            controller.cadastrarCachorro("", 2, "SRD", 10, TipoPelagem.CURTA, PorteAnimal.PEQUENO, false);
        }).toThrow("O nome não pode ser vazio.");
    });

    test("busca animal por nome (sobrecarga com string)", () => {
        controller.cadastrarCachorro("Rex", 3, "Labrador", 30, TipoPelagem.CURTA, PorteAnimal.GRANDE, true);
        controller.cadastrarGato("Mimi", 2, "Siamês", 4, TipoPelagem.CURTA, false);
    ...
    test("ordena animais por idade (crescente)", () => {
        controller.cadastrarCachorro("Rex", 5, "Labrador", 30, TipoPelagem.CURTA, PorteAnimal.GRANDE, true);
        controller.cadastrarGato("Mimi", 1, "Siamês", 4, TipoPelagem.CURTA, false);
    ...
    test("polimorfismo: cada animal tem seus cuidados recomendados", () => {
        const cachorro = controller.cadastrarCachorro("Rex", 3, "Labrador", 30, TipoPelagem.CURTA, PorteAnimal.GRANDE, true);
        const peixe = controller.cadastrarPeixe("Nemo", 1, "Palhaço", 0.1, TipoAgua.SALGADA);
    ...
    test("regra: um animal não pode ter dois donos", () => {
        ...
        expect(() => controller.vincularAnimalAoCliente(ana, cachorro)).toThrow(ValidacaoException);
    });
    ...
    test("regra: só dá para agendar serviço para um animal do próprio cliente", () => {
        ...
        expect(() => controller.agendarServico(TipoServico.BANHO, ana, cachorro)).toThrow(ValidacaoException);
    });
    ...
    test("preço do serviço é calculado: preço-base x fator do animal", () => {
        ...
        const cliente = controller.cadastrarCliente("João", "99999-9999");
        controller.vincularAnimalAoCliente(cliente, cachorro);
        const servico = controller.agendarServico(TipoServico.BANHO, cliente, cachorro);
    ...
    test("regra de serviço: peixe não pode tomar banho", () => {
        ...
        expect(() => controller.agendarServico(TipoServico.BANHO, cliente, peixe)).toThrow(ValidacaoException);
    });
    ...
    test("répteis aceitam apenas consulta e vacina", () => {
        ...
        expect(lagarto.servicosDisponiveis()).toEqual([TipoServico.CONSULTA, TipoServico.VACINA]);
        expect(tartaruga.servicosDisponiveis()).toEqual([TipoServico.CONSULTA, TipoServico.VACINA]);
    });

    test("status: concluir um serviço alimenta o faturamento realizado", () => {
        const cachorro = controller.cadastrarCachorro("Rex", 3, "Labrador", 30, TipoPelagem.CURTA, PorteAnimal.MEDIO, true);
        const cliente = controller.cadastrarCliente("João", "99999-9999");
        controller.vincularAnimalAoCliente(cliente, cachorro);
        const servico = controller.agendarServico(TipoServico.BANHO, cliente, cachorro); // 50 x 1.3 = 65
    ...
    test("status: serviço cancelado não conta no faturamento previsto", () => {
        ...
        const cliente = controller.cadastrarCliente("João", "99999-9999");
        controller.vincularAnimalAoCliente(cliente, cachorro);
        const s1 = controller.agendarServico(TipoServico.BANHO, cliente, cachorro); // 65
        controller.agendarServico(TipoServico.TOSA, cliente, cachorro);             // 52
    ...
    test("relatório: valor médio por serviço considera serviços não cancelados", () => {
        ...
        const cliente = controller.cadastrarCliente("João", "99999-9999");
        controller.vincularAnimalAoCliente(cliente, cachorro);
        controller.agendarServico(TipoServico.BANHO, cliente, cachorro); // 50 x 1.3 = 65
        controller.agendarServico(TipoServico.TOSA, cliente, cachorro);  // 40 x 1.3 = 52
        // (65 + 52) / 2 = 58.5
        expect(controller.valorMedioPorServico()).toBeCloseTo(58.5);
    });

    test("relatório: serviço mais procurado (ignora cancelados)", () => {
        const gato = controller.cadastrarGato("Mimi", 2, "Siamês", 4, TipoPelagem.CURTA, false);
        const cliente = controller.cadastrarCliente("Ana", "8888-8888");
        controller.vincularAnimalAoCliente(cliente, gato);
        controller.agendarServico(TipoServico.BANHO, cliente, gato);
        controller.agendarServico(TipoServico.BANHO, cliente, gato);
        controller.agendarServico(TipoServico.TOSA, cliente, gato);
        expect(controller.servicoMaisProcurado()).toBe(TipoServico.BANHO);
    });
});
